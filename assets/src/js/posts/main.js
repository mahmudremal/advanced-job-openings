...
( function ( $ ) {
	...
	class FUTUREWORDPRESS_PROJECT_FROTEND_MAIN {
      ...
      // this.according();this.progress();
		...
		init() {
      this.cv_add();this.cv_edit();this.cv_delete();this.apply_delete();this.job_delete();this.dataTable();
    }
    cv_add() {
    ...
    job_delete() {
      const thisClass = this;
      document.querySelectorAll( '.approve-job-fwp' ).forEach( function( e, i ) {
        e.addEventListener( 'click', function( event ) {
            var id = ( this.dataset.id ) ? this.dataset.id : false;
            var nonce = ( this.dataset.nonce ) ? this.dataset.nonce : '';
            ...
            if( id ) {// && confirm( thisClass.confirmDeleteJob ) 
              var data = { action: 'fwp-company-approve-job-action', job: id, 'fwp-company-approve-job-action': nonce };
              $.ajax( {
                url: thisClass.ajaxUrl,
                type: 'POST',
                data: data,
