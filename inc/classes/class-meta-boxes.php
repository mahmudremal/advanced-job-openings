...
class Meta_Boxes {
   ...
   */
	...
	public function admin_init() {
		add_meta_box( 'fwp_job_vanues-group', __( 'Job dates and Locations', FUTUREWORDPRESS_PROJECT_TEXT_DOMAIN ), [ $this, 'repeater' ], 'job_openings', 'normal', 'default');
			...
			<tbody>
				...
				if ( $fwp_job_vanues_group ) :
					foreach ( $fwp_job_vanues_group as $field ) {
						...
						<tr>
							<td class="job-row">
								<input class="jobvanuedate" type="date" placeholder="<?php esc_attr_e( 'Date', FUTUREWORDPRESS_PROJECT_TEXT_DOMAIN ); ?>" title="<?php esc_attr_e( 'Date', FUTUREWORDPRESS_PROJECT_TEXT_DOMAIN ); ?>" name="JobDate[]" value="<?php if($field['JobDate'] != '') echo esc_attr( $field['JobDate'] ); ?>" />
				...
				<tr>
					<td class="job-row">
						<input class="jobvanuedate" type="date" placeholder="<?php esc_attr_e( 'Date', FUTUREWORDPRESS_PROJECT_TEXT_DOMAIN ); ?>" title="<?php esc_attr_e( 'Date', FUTUREWORDPRESS_PROJECT_TEXT_DOMAIN ); ?>" name="JobDate[]" />
						<select class="jobvanuelocation" name="JobLocation[]">
							<?php foreach( $jobLocations as $location ) : ?>
				...
				<td class="job-row">
					...
					<td><a class="button remove-row" href="#"><?php esc_html_e( 'Remove', FUTUREWORDPRESS_PROJECT_TEXT_DOMAIN ); ?></a></td>
				</tr>
			</tbody>
		</table>
		<p><a id="add-row" class="button" href="#"><?php esc_html_e( 'Add another', FUTUREWORDPRESS_PROJECT_TEXT_DOMAIN ); ?></a></p>
		<style>
			.job-row {
				display: flex;
				flex-wrap: wrap;
  ...
  public function save_post( $post_id ) {
	...
	private function media_button( $field ) {
		...
		printf(
			...
			isset( $field['modal-button'] ) ? $field['modal-button'] : __( 'Select this file', FUTUREWORDPRESS_PROJECT_TEXT_DOMAIN ),
			isset( $field['modal-title'] ) ? $field['modal-title'] : __( 'Choose a file', FUTUREWORDPRESS_PROJECT_TEXT_DOMAIN ),
			$field['return'],
			$field['id'], $field['id'],
			isset( $field['button-text'] ) ? $field['button-text'] : __( 'Upload', FUTUREWORDPRESS_PROJECT_TEXT_DOMAIN )
		);
	}
