    ...
    if( isset( $post->post_status ) && isset( $newMeta[ 'jobs' ][ 'positionfilled' ] ) ) {
      $newMeta[ 'jobs' ][ '_status' ] = ( $post->post_status == 'publish' ) ? (
        ( ! $newMeta[ 'jobs' ][ 'positionfilled' ] ) ? (
          ( ! $newMeta[ 'jobs' ][ 'is_expired' ] ) ? true : false
        ) : false
      ) : false;
      $newMeta[ 'jobs' ][ '_statusText' ] = ( $newMeta[ 'jobs' ][ '_status' ] ) ? __( 'Active', FUTUREWORDPRESS_PROJECT_TEXT_DOMAIN ) : ( ( $newMeta[ 'jobs' ][ 'is_expired' ] ) ? __( 'Expired', FUTUREWORDPRESS_PROJECT_TEXT_DOMAIN ) : __( 'Inactive', FUTUREWORDPRESS_PROJECT_TEXT_DOMAIN ) );
    }
